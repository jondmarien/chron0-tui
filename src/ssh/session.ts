...
export function startInteractiveSession(
  ...
  const instance = render(React.createElement(App, { visitor: meta.username }), {
  ...
  channel.on('window-change' as any, onWindowChange);
...
export function writeTextSnapshot(channel: ServerChannel): void {
  ...
  lines.push('');
  lines.push('For the full interactive site, connect with a PTY:  ssh ssh.chron0.tech');
  lines.push('');
